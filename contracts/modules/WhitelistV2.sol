...
abstract contract WhitelistV2 is IWhitelist {
  ...
  // @return - bool if whitelist is enabled or not
  // @param myAddress - any user account address, EOA or contract
  function myWhitelistStatus(address myAddress) external view override(IWhitelist) returns (bool) {
    return whitelist.onList(myAddress);
  }

  // @dev will return status of whitelist
  // @return - bool if whitelist is enabled or not
  function whitelistStatus() external view override(IWhitelist) returns (bool) {
  ...
  //  a timestamp
  function whitelistEnd() external view override(IWhitelist) returns (uint) {
    return whitelist.showEnd();
  }

  // @dev will return totat on whitelist
  // @return - uint from CountersV2.Count
  function TotalOnWhitelist() external view override(IWhitelist) returns (uint) {
    return whitelist.totalAdded();
  }

  // @dev will return totat used on whitelist
  // @return - uint from CountersV2.Count
  function TotalWhiteListUsed() external view override(IWhitelist) returns (uint) {
    return whitelist.totalRemoved();
  }

  // @dev will return totat used on whitelist
  // @return - uint aka xxxx = xx.xx%
  function WhitelistEfficiency() external view override(IWhitelist) returns (uint) {
  ...
  function supportsInterface(bytes4 interfaceId) public view override(IWhitelist, IERC165) returns (bool) {
    return (
